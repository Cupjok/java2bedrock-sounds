...
# --- [ INITIAL CLEANUP ] ---
# Clean up previous conversion files to ensure a clean start for unzipping, conversion, and packaging.
rm -rf assets ia_overlay_* scratch_files target && rm -f pack.mcmeta pack.png config.json sprites.json default_assets.zip
# ---------------------------
...
# Check OS and define command for fetching the number of CPU Cores
case "$(uname -s)" in
    Linux*|CYGWIN*|MINGW32*|MSYS*)
        # For Linux and Windows systems (via Cygwin/MinGW): use 'nproc' (or fallback)
        CPU_CORE_CMD="nproc"
        if ! command -v nproc &> /dev/null; then
            CPU_CORE_CMD="grep -c ^processor /proc/cpuinfo"
        fi
        ;;
    Darwin*)
        # For macOS (Darwin systems): use the 'sysctl' command
        CPU_CORE_CMD="sysctl -n hw.ncpu"
        ;;
    *|*)
        # For all other systems, use a default value of 1
        CPU_CORE_CMD="echo 1"
        ;;
esac
export CPU_CORE_CMD
...
# MODIFIED FOR UNIVERSAL CPU CORE COUNTING: Uses dynamically detected command from CPU_CORE_CMD
wait_for_jobs () {
...
rm -rf assets ia_overlay_* scratch_files target && rm -f pack.mcmeta && rm -f pack.png && rm -f config.json sprites.json default_assets.zip
...
# cleanup
rm -rf assets ia_overlay_* && rm -f pack.mcmeta && rm -f pack.png && rm -f config.json sprites.json default_assets.zip
...
# Calculate to H:MM:SS
H=$((DURATION / 3600))
M=$(((DURATION % 3600) / 60))
S=$((DURATION % 60))

status_message info "---------------------------------------------------------"
status_message completion "Conversion Complete! Total time: ${H}h ${M}m ${S}s"
status_message info "---------------------------------------------------------"
# --------------------------------------------------------------------------
