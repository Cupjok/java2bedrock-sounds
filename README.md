...
- `-m (pack_to_merge.mcpack)` filename of Bedrock Edition resource pack in the same directory to merge with the result of the conversion
...
```
### To convert sounds only (converter_sounds_only.sh)

To run, simply:
```
./converter_sounds_only.sh MyResourcePack.zip
```
...
- [jq](https://stedolan.github.io/jq/download/) (1.6/1.7)
...
### Debian, Ubuntu, & Mint
```sh
sudo apt-get install moreutils jq imagemagick unzip zip nodejs uuid-runtime ffmpeg
npm i -g spritesheet-js
```

### MacOS
```sh
brew install moreutils jq imagemagick unzip zip nodejs uuid-runtime ffmpeg
npm i -g spritesheet-js
```

### RHEL, Fedora, & Centos
```sh
sudo yum install moreutils jq imagemagick unzip zip nodejs uuid-runtime ffmpeg
npm i -g spritesheet-js
```

### Arch Linux
```sh
sudo pacman -S moreutils jq imagemagick unzip zip nodejs uuid-runtime ffmpeg
npm i -g spritesheet-js
```
